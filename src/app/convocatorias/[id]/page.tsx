...
// ── Modal de guía enriquecida (slides gallery) ──────────────────────────────
...
function GuiaModal({ guia, onClose }: { guia: GuiaSubvencion; onClose: () => void }) {
  const summary = guia?.grant_summary;
  const docs: string[] = guia?.required_documents ?? [];
  const reqs: string[] = guia?.universal_requirements_lgs_art13 ?? [];
  const methods = guia?.application_methods ?? [];
  const workflows = guia?.workflows ?? [];
  const disclaimer = guia?.legal_disclaimer ?? "";
          ...
          <div className="space-y-2.5">
            {summary.title && (
              <div className="rounded-xl border border-border/50 bg-surface-muted/30 p-3">
                <p className="text-[10px] font-semibold uppercase tracking-wider text-foreground-muted mb-0.5">Título</p>
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border bg-surface-muted/50">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-sm font-bold text-foreground">Guía de solicitud</p>
...
function AnalisisIaCard({ convocatoriaId }: { convocatoriaId: number }) {
  ...
  const [guia, setGuia] = useState<GuiaSubvencion | null>(null);
  ...
  const analizar = async () => {
    ...
    try {
      ...
      const ax = err as { response?: { status?: number; data?: unknown }; code?: string; message?: string };
      console.error("Error análisis FULL:", JSON.stringify({ status: ax?.response?.status, code: ax?.code, data: ax?.response?.data, msg: ax?.message }));
      const d = ax?.response?.data as Record<string, string> | undefined;
      const msg = d?.message || d?.error || (ax?.code === "ECONNABORTED" ? "Timeout: el análisis tardó demasiado." : ax?.message) || "Error al analizar.";
      setError(msg);
    } finally {
      setAnalizando(false);
    }
  ...
  const verGuia = async () => {
    ...
    if (guia) { setShowGuia(true); return; }
    setCargandoGuia(true);
    try {
      const res = await convocatoriasUsuarioApi.guia(convocatoriaId);
      setGuia(res.data);
      setShowGuia(true);
    } catch (err: unknown) {
      const ax = err as { response?: { status?: number; data?: { error?: string; message?: string } }; message?: string };
      const msg = ax?.response?.data?.error || ax?.response?.data?.message || ax?.message || "Error al cargar la guía.";
      console.warn("Error guía:", ax?.response?.status, msg);
      setError(msg);
    } finally {
  ...
  return (
    <>
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => setShowGallery(true)}
                className="flex-1 inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-white hover:bg-primary-hover transition-colors">
                <BookOpen className="w-4 h-4" /> Ver análisis
              </button>
              <button type="button" onClick={verGuia} disabled={cargandoGuia}
                className="flex-1 inline-flex items-center justify-center gap-2 rounded-xl border-2 border-primary px-4 py-2.5 text-sm font-semibold text-primary hover:bg-primary/5 transition-colors disabled:opacity-50">
                {cargandoGuia ? (
                  <><Loader2 className="w-4 h-4 animate-spin" /> Cargando...</>
                ) : (
                  <><FileText className="w-4 h-4" /> Ver guía</>
                )}
              </button>
            </div>
          </div>
      ...
      {showGuia && guia && (
        <GuiaModal guia={guia} onClose={() => setShowGuia(false)} />
      )}
    </>
  );
}
