...
export default function AdminUsuariosPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-foreground mb-6">Usuarios ({usuarios.length})</h1>
      <div className="bg-surface rounded-2xl border border-border shadow-sm overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-surface-muted border-b border-border">
            <tr>
              <th className="text-left px-4 py-3 font-medium text-foreground-muted">Email</th>
              <th className="text-left px-4 py-3 font-medium text-foreground-muted">Rol</th>
              <th className="text-left px-4 py-3 font-medium text-foreground-muted hidden sm:table-cell">Registro</th>
            ...
            {usuarios.map((u) => (
              <tr key={u.id} className="hover:bg-surface-muted transition-colors">
                ...
                <td className="px-4 py-3 text-foreground-muted hidden sm:table-cell">
                  {u.creadoEn ? new Date(u.creadoEn).toLocaleDateString("es") : "-"}
                </td>
